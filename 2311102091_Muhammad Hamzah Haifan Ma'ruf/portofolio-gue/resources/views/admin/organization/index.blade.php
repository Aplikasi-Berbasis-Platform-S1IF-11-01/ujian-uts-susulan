...
@section('content')
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:25px; gap:20px; flex-wrap:wrap;">
        <div>
            <h1 style="margin:0; font-size:30px; color:#fff;">Organization</h1>
            <p style="margin-top:8px; color:#9ca3af;">Kelola data organization pada landing page.</p>
        </div>
        ...
        <table style="width:100%; border-collapse:collapse;">
            <thead style="background:#202020;">
                <tr>
                    ...
                    <th style="padding:16px; text-align:left; color:#fff;">Organization Name</th>
                    <th style="padding:16px; text-align:left; color:#fff;">Role</th>
            ...
            <tbody>
                @forelse($organizations as $index => $organization)
                    <tr style="border-top:1px solid #2e2e2e;">
                        ...
                        <td style="padding:16px; color:#fff;">{{ $organization->organization_name }}</td>
                        <td style="padding:16px; color:#d1d5db;">{{ $organization->role }}</td>
                        <td style="padding:16px; color:#d1d5db;">{{ $organization->year }}</td>
                        <td style="padding:16px; color:#d1d5db;">{{ $organization->description ?: '-' }}</td>
                        <td style="padding:16px; text-align:center;">
                            <div style="display:flex; justify-content:center; align-items:center; gap:10px; flex-wrap:nowrap;">
                                ...
                                <form action="{{ route('admin.organization.destroy', $organization->id) }}"
                                      method="POST"
                                      onsubmit="return confirm('Yakin ingin menghapus data organization ini?')"
                                      style="margin:0;">
                                    @csrf
                                    @method('DELETE')
